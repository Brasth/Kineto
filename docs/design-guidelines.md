...
- **Floating caption overlay:** a linked pair of automatic nonactivating panels during active capture only (`AppModel.capturePresentationMode == .floating`). Uses a distinct higher-weight top status zone (prominent REC indicator + controls) with refined mint left signal accent over a comfortable transcript zone. Brief commanding entrance treatment on first appearance after Start that settles quickly. The pair shares one anchor, display clamp, lifecycle, Reset Caption Position action, and normal screenshot/screen-share visibility. Only the transcript anchor persists. Successful Start Meeting enters floating mode automatically.
